...
**実行方法**:
1. **制約の特定**: 現在の課題における主要な制約条件をリストアップする
2. **仮想的消去**: 「もしこの制約がなかったら？」と各制約を一時的に取り除いて考える
3. **理想解の探索**: 制約なしの状態で考えられる理想的な解決策を自由に発想する
4. **現実への適応**: 理想解を現実の制約下で部分的にでも実現する方法を探る
5. **新選択肢の追加**: 制約を緩和・回避・克服する創造的な選択肢を追加する
